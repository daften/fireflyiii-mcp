...
- `scripts/changelog-guard.mjs`, which fails the build when `CHANGELOG.md`'s `merge=union` driver has resolved a merge cleanly but wrongly. Union keeps both sides of a conflicting hunk by concatenating lines, with no notion of which release section a bullet belongs to, so it can splice a stale branch's entries into an already-published section, absorb pending `[Unreleased]` entries into the release that just shipped (emptying `[Unreleased]` in the process), or leave two `### Fixed` headings in one section — all reported by git as a clean merge. The guard treats released sections as immutable, comparing each against the tag that published it, and runs on pull requests, on pushes (back-merges reach `develop` without a PR), and again before publishing.
