...
export async function fetchRecurrence(client, id) {
    const response = await client.get(`/recurrences/${id}`);
    return unwrapSingle(response);
}
export async function createRecurrence(client, params) {
    const repetition = { type: params.repeat_type, moment: params.repeat_moment };
    if (params.skip !== undefined)
        repetition.skip = params.skip;
    if (params.weekend !== undefined)
        repetition.weekend = params.weekend;
    const transaction = {
    ...
    if (params.currency_code !== undefined)
        transaction.currency_code = params.currency_code;
    if (params.tags !== undefined)
        transaction.tags = params.tags;
    if (params.transaction_notes !== undefined)
        transaction.notes = params.transaction_notes;
    const body = {
        type: params.type,
        title: params.title,
    ...
    if (params.nr_of_repetitions !== undefined)
        body.nr_of_repetitions = params.nr_of_repetitions;
    const response = await client.post('/recurrences', body);
    return unwrapSingle(response);
}
export async function updateRecurrence(client, id, params) {
    const body = {};
    ...
    if (params.notes !== undefined)
        body.notes = params.notes;
    if (params.first_date !== undefined)
        body.first_date = params.first_date;
    if (params.repeat_until !== undefined)
        body.repeat_until = params.repeat_until;
    if (params.nr_of_repetitions !== undefined)
        body.nr_of_repetitions = params.nr_of_repetitions;
    if (params.apply_rules !== undefined)
        body.apply_rules = params.apply_rules;
    ...
    if (hasRepetitionFields) {
        const repetition = {};
        if (params.repeat_type !== undefined)
            repetition.type = params.repeat_type;
        if (params.repeat_moment !== undefined)
            repetition.moment = params.repeat_moment;
        if (params.skip !== undefined)
            repetition.skip = params.skip;
        if (params.weekend !== undefined)
            repetition.weekend = params.weekend;
    ...
    if (hasTransactionFields) {
        ...
        if (params.transaction_description !== undefined)
            transaction.description = params.transaction_description;
        if (params.source_id !== undefined)
            transaction.source_id = params.source_id;
    ...
    const response = await client.put(`/recurrences/${id}`, body);
    return unwrapSingle(response);
}
export async function deleteRecurrence(client, id) {
