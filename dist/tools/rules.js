...
const triggerTypeSchema = z.string().describe('Trigger type. Common values: from_account_starts, from_account_ends, from_account_is, from_account_contains, to_account_starts, to_account_ends, to_account_is, to_account_contains, amount_less, amount_exactly, amount_more, description_starts, description_ends, description_contains, description_is, transaction_type, category_is, budget_is, tag_is, currency_is, has_attachments, has_no_category, has_any_category, has_no_budget, has_any_budget, has_no_tag, has_any_tag, notes_contains, notes_starts, notes_end, notes_are, no_notes, any_notes, source_account_is, destination_account_is, source_account_starts');
const actionTypeSchema = z.string().describe('Action type. Common values: user_action, set_category, clear_category, set_budget, clear_budget, add_tag, remove_tag, remove_all_tags, set_description, append_description, prepend_description, set_source_account, set_destination_account, set_notes, append_notes, prepend_notes, clear_notes, link_to_bill, convert_withdrawal, convert_deposit, convert_transfer, delete_transaction');
...
export function registerRuleTools(server, client) {
    ...
    }, ({ title, description, active }) => createRuleGroup(client, { title: title, description: description, active: active }));
    ...
    }, ({ id, title, description, active }) => updateRuleGroup(client, id, { title: title, description: description, active: active }));
    ...
    defineTool(server, 'get_rule_group_rules', {
        ...
        annotations: READ_ANNOTATIONS,
    }, ({ id, page, limit }) => fetchRuleGroupRules(client, id, { page: page, limit: limit }));
    ...
    }, ({ id, start, end, accounts }) => triggerRuleGroup(client, id, { start: start, end: end, accounts: accounts }));
    defineTool(server, 'trigger_rule', {
        ...
        annotations: { openWorldHint: true },
    }, ({ id, start, end, accounts }) => triggerRule(client, id, { start: start, end: end, accounts: accounts }));
