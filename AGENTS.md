...
- **The union driver resolves cleanly but not correctly, so `scripts/changelog-guard.mjs` checks the result.** Union concatenates conflicting *lines*; it has no idea which release section a bullet belongs to. Three shapes have actually occurred, every one reported by git as a clean merge: a stale branch replaying its changelog snapshot into an already-published section (sometimes duplicating an entry verbatim); a back-merge absorbing `develop`'s pending `[Unreleased]` entries into the dated section that just shipped, so the release claims changes it never contained *and* those entries vanish from every future release's notes; and two branches each adding a `### Fixed` heading, leaving two of them. The guard enforces that **released sections are immutable** — each is compared against the tag that published it, not against the base branch — and that no still-editable section has a duplicate heading. It runs in `ci.yml` on pull requests *and* pushes (back-merges reach `develop` without a PR), and again in `publish.yml`'s `verify` job, since the GitHub Release body is extracted from these sections. A deliberate edit to a published section needs `[skip changelog-guard]` in the PR title. Already-published duplicate headings are left alone on purpose — `0.4.6` shipped with two `### Security` headings, and the immutability rule forbids the fix.
