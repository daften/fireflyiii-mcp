...
2. `auto-merge.yml` enables auto-merge (merge commit); branch protection's required checks gate the merge. It only does so for PRs that fix a real advisory, which it establishes with `dependabot/fetch-metadata`'s `alert-lookup`. That lookup needs a token with **`Dependabot alerts: Read only`** — the default `GITHUB_TOKEN` cannot read the alerts API, and the action signals that by returning an *empty* `alert-state` rather than failing, which is indistinguishable from "not a security PR". So `RELEASE_TOKEN` is passed to the metadata step too, and a guard step fails the job when a `security-fixes`-group PR yields no `alert-state` (the shape an expired or under-scoped PAT takes).
