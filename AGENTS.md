...
- **`main`** — default branch, always releasable. Receives only: Dependabot security PRs (auto-merged), manual hotfix PRs, promotion merges from `develop`, and the `chore(release): X.Y.Z` commit itself. Never commit anything else directly to `main` — see "Releasing a New Version" for why the release commit is the exception.
...
**Which branch each step happens on matters.** `## [Unreleased]` accumulates on `develop`. The dated `## [X.Y.Z]` section, the `package.json` version bump, and the tag are **all created on `main`**, after the promotion merge — never on `develop`. Two reasons: a version section sitting on `develop` claims a release that hasn't shipped, and since `backmerge.yml` carries `main` back into `develop` after every release, promoting on `develop` would make that back-merge conflict with itself. The `chore(release): X.Y.Z` commit is the one sanctioned exception to "never commit directly to `main`" — `auto-release.yml` does exactly this for automated security releases.
...
6. Create an annotated git tag whose message is the same `## [X.Y.Z]` block from `CHANGELOG.md` (a git-native record of what shipped). The publish workflow validates that this section exists before publishing.
7. Push the commit and then the tag: `git push origin main && git push origin v<version>` — the tag triggers the publish workflow, which validates the changelog, runs tests, publishes to npm + GHCR, and creates a GitHub Release whose notes are extracted from the matching `CHANGELOG.md` section.
8. `backmerge.yml` opens the `main` → `develop` PR that carries the released section back. If `develop` has already collected new `[Unreleased]` entries, this conflicts — routine, not a malfunction. Resolve by keeping both sides.
