...
9. `backmerge.yml` opens the single `main` → `develop` PR that carries the released section back. If `develop` has already collected new `[Unreleased]` entries, this conflicts — routine, not a malfunction. Resolve by keeping both sides. Delete the release branch once it's merged.
