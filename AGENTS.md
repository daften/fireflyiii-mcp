...
- **A back-merge PR appearing means the merge conflicted** and needs manual resolution — that's the only path that opens one, and the job fails so it surfaces. During active development this is *routine*, not a malfunction: releases and feature PRs both edit the top of `CHANGELOG.md`. Resolve by keeping both sides — the released `## [X.Y.Z]` section and the remaining `[Unreleased]` entries. Still, one push to `main` means one back-merge, which is why a release is shaped as a single `release/X.Y.Z` PR rather than a promotion merge followed by a release commit.
...
9. `backmerge.yml` merges the released section straight back into `develop`. If `develop` has already collected new `[Unreleased]` entries this conflicts, and it opens a PR instead — routine, not a malfunction. Resolve by keeping both sides. Delete the release branch once it's merged.
