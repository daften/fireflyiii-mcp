...
export function registerBudgetTools(server: McpServer, client: FireflyClient): void {
  ...
  defineTool(server, 'get_available_budgets', {
    ...
    annotations: READ_ANNOTATIONS,
  }, ({ page, limit }) => fetchAvailableBudgets(client, { page: page as number | undefined, limit: limit as number | undefined }));
  ...
  defineTool(server, 'get_budget_transactions', {
    ...
    fetchBudgetTransactions(client, id as string, { start, end, page, limit } as Parameters<typeof fetchBudgetTransactions>[2]));
  ...
  defineTool(server, 'get_transactions_without_budget', {
    ...
    fetchTransactionsWithoutBudget(client, params as Parameters<typeof fetchTransactionsWithoutBudget>[1]));
}
